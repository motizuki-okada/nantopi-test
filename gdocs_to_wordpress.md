...
| `.chat-icon` | アイコンエリア |
| `.chat-bubble` | 吹き出しエリア |
| `.chat-icon-img` | コンテナなしのアイコン画像（円形50px） |
...
[id^="toc-heading-"] { scroll-margin-top: 100px; }
...
### 必須
- [x] `.toc-container`, `.toc-title`, `.toc-level-*`
- [x] `.chat-block`, `.chat-row`, `.chat-right`, `.chat-icon`, `.chat-bubble`
- [x] `.wp-block-heading` + アンカーリンク対応
- [x] `.wp-block-image.size-large`
- [x] `.wp-block-quote`（引用ブロック）
